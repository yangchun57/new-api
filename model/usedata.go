...
func GetQuotaDataByTokenId(userId int, tokenId int, startTime int64, endTime int64) (quotaData []*QuotaData, err error) {
	var quotaDatas []*QuotaData
	err = DB.Table("quota_data").
		Select("model_name, created_at, sum(count) as count, sum(quota) as quota, sum(token_used) as token_used").
		Where("user_id = ? and token_id = ? and created_at >= ? and created_at <= ?", userId, tokenId, startTime, endTime).
